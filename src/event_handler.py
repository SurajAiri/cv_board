...
class HandEventHandler:
    def __init__(self,onLeftClick=None):
        ...
        self.draw_option:DrawOption = DrawOption.LINE

    def update_index_tip(self, x,y):
        # self.prev_pos = self.pos
        self.pos = (math.ceil(x*1920),math.ceil(y*1080))
    ...
    def event_handler(self, hand_landmarks):
        ...
        self.index_active = self.is_finger_straight(wrist, index_finger_joints)
        self.middle_active = self.is_finger_straight(wrist, middle_finger_joints)

        if self.index_active and self.middle_active and self.__is_left_click__(wrist, index_finger_joints, middle_finger_joints):
            self.left_pressed = True

            # Call the left click event handler
            if self.onLeftClick:
                self.onLeftClick()
        else:
            self.left_pressed = False
        ...
        self.update_index_tip(index_finger_joints[0][0],index_finger_joints[0][1])
